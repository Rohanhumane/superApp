...
interface Props extends TextInputProps {
  ...
  rightIcon?: React.ReactNode; onRightIconPress?: () => void; disabled?: boolean;
}

export const Input: React.FC<Props> = ({ label, error, required, rightIcon, onRightIconPress, disabled, style, onFocus, onBlur, ...rest }) => {
  ...
  return (
    <View style={{ marginBottom: sp.base }}>
      ...
      <View style={[s.box, { borderColor }, disabled && { opacity: 0.6 }]}>
        <TextInput style={[s.input, style]} placeholderTextColor={colors.text.secondary} editable={!disabled}
          onFocus={e => { setFocused(true); onFocus?.(e); }} onBlur={e => { setFocused(false); onBlur?.(e); }} {...rest} />
        {rightIcon && <TouchableOpacity onPress={onRightIconPress} style={{ padding: sp.sm }}>{rightIcon}</TouchableOpacity>}
      </View>
